...
export function registerTicketTools(server, client) {
    server.tool("list_tickets", "List tickets from Atera helpdesk. Supports filtering by status and customer.", {
        ...
        try {
            const result = await client.getList("/api/v3/tickets", args);
    ...
    server.tool("get_ticket", "Get a specific ticket by ID with full details.", {
        ticketId: z.number().describe("The ticket ID"),
    }, async ({ ticketId }) => {
        try {
            const result = await client.get(`/api/v3/tickets/${ticketId}`);
            return formatResponse(result);
        }
        catch (error) {
            return formatError(error);
    ...
    server.tool("create_ticket", "Create a new ticket. Requires title and description. Specify end user by ID or by email (with first/last name for new users).", {
    ...
    server.tool("update_ticket", "Update an existing ticket. Change status, priority, assignment, title, etc. Use this to close, resolve, reassign, or modify tickets.", {
    ...
    server.tool("list_ticket_comments", "Get all comments/replies on a ticket.", {
        ticketId: z.number().describe("The ticket ID"),
        page: z.number().optional().describe("Page number (default 1)"),
        itemsInPage: z.number().optional().describe("Items per page (default 20, max 50)"),
    }, async ({ ticketId, ...params }) => {
        try {
            const result = await client.getList(`/api/v3/tickets/${ticketId}/comments`, params);
            return formatResponse(result);
        }
        catch (error) {
            return formatError(error);
    ...
    server.tool("get_ticket_billable_duration", "Get billable time duration for a ticket.", {
        ticketId: z.number().describe("The ticket ID"),
    }, async ({ ticketId }) => {
        try {
            const result = await client.get(`/api/v3/tickets/${ticketId}/billableduration`);
            return formatResponse(result);
        }
        catch (error) {
            return formatError(error);
    ...
    server.tool("get_ticket_nonbillable_duration", "Get non-billable time duration for a ticket.", {
        ticketId: z.number().describe("The ticket ID"),
    }, async ({ ticketId }) => {
        try {
            const result = await client.get(`/api/v3/tickets/${ticketId}/nonbillableduration`);
            return formatResponse(result);
        }
        catch (error) {
            return formatError(error);
    ...
    server.tool("get_ticket_work_hours", "Get work hours duration for a ticket.", {
        ticketId: z.number().describe("The ticket ID"),
    }, async ({ ticketId }) => {
        try {
            const result = await client.get(`/api/v3/tickets/${ticketId}/workhours`);
            return formatResponse(result);
        }
        catch (error) {
            return formatError(error);
    ...
    server.tool("get_ticket_work_hours_records", "Get detailed work hour records/entries for a ticket.", {
        ticketId: z.number().describe("The ticket ID"),
        page: z.number().optional().describe("Page number (default 1)"),
        itemsInPage: z.number().optional().describe("Items per page (default 20, max 50)"),
    }, async ({ ticketId, ...params }) => {
        try {
            const result = await client.getList(`/api/v3/tickets/${ticketId}/workhoursrecords`, params);
            return formatResponse(result);
        }
        catch (error) {
            return formatError(error);
    ...
    server.tool("list_resolved_closed_tickets", "List tickets that have been resolved or closed, filtered by date range.", {
        page: z.number().optional().describe("Page number (default 1)"),
        itemsInPage: z.number().optional().describe("Items per page (default 20, max 50)"),
    }, async (args) => {
        try {
            const result = await client.getList("/api/v3/tickets/statusmodified", args);
            return formatResponse(result);
        }
        catch (error) {
            return formatError(error);
    ...
    server.tool("delete_ticket", "Delete a ticket by ID.", {
